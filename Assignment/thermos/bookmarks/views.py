...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(bm)
        db.session.commit()
        flash("Stored bookmark: {0} {1}".format(url, description))
...
def edit_bookmark(bookmark_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Edited bookmark: {0} {1}".format(bookmark.url, bookmark.description))
        return redirect(url_for('bookmarks.user', username = current_user.username))
    return render_template('bookmark_form.html', form=form, title='Edit a bookmark')
...
def delete_bookmark(bookmark_id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash("Deleted bookmark: {0}".format(bookmark.description))
        return redirect(url_for('bookmarks.user', username = current_user.username))
    else :
        flash(('Please confirm before deleting the message.'))
